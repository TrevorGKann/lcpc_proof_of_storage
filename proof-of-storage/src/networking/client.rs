...
use lcpc_2d::{open_column, FieldHash, LcCommit, LcEncoding, LcRoot, ProverError};
...
use crate::fields::{
    convert_byte_vec_to_field_elements_vec, convert_field_elements_vec_to_byte_vec,
    evaluate_field_polynomial_at_point, evaluate_field_polynomial_at_point_with_elevated_degree,
};
use crate::lcpc_online::{
    client_verify_commitment, convert_file_data_to_commit, get_PoS_soudness_n_cols,
    hash_column_to_digest, server_retreive_columns, CommitDimensions, CommitOrLeavesOutput,
    CommitRequestType, FldT,
};
use crate::networking::server;
use crate::networking::server::{
    get_aspect_ratio_default_from_field_len, get_aspect_ratio_default_from_file_len,
};
...
pub async fn upload_file(
        ...
        match (num_pre_encoded_columns, num_encoded_columns) {
            (Some(num_pre_encoded_columns), Some(num_encoded_columns)) => {
                ...
                (
                    ...
                    num_encoded_columns,
                    crate::networking::server::get_soundness_from_matrix_dims(
                        num_pre_encoded_columns,
                        num_encoded_columns,
                    ),
                )
            ...
            (Some(cols), None) => {
                ...
                (
                    cols,
                    enc_cols,
                    crate::networking::server::get_soundness_from_matrix_dims(cols, enc_cols),
                )
            }
            (None, Some(enc_cols)) => {
                ensure!(
                    enc_cols >= 2,
                    "Number of encoded columns must be greater than 2"
                );
                ensure!(
                    enc_cols.is_power_of_two(),
                    "Number of encoded columns must be a power of 2"
                );
    ...
    let CommitOrLeavesOutput::Leaves(locally_derived_leaves) =
        convert_file_data_to_commit::<Blake3, WriteableFt63>(
            &field_file_data,
    ...
    let to_send = ClientMessages::UploadNewFile {
        filename: file_name,
        file: file_data,
    ...
    // error: not sure why there's two unwraps here
    // file_metadata.stored_server.server_ip = server_ip
    //     .clone()
    //     .split(":")
    //     .next()
    //     .unwrap_or("")
    //     .to_string();
    file_metadata.stored_server.server_port = server_ip
        .split(":")
        .nth(1)
    ...
    sink.send(ClientMessages::RequestProof {
        ...
        columns_to_verify: cols_to_verify.clone(),
    })
    .await?;
    ...
    db.create::<Option<FileMetadata>>((
        constants::CLIENT_METADATA_TABLE,
        file_metadata.id_ulid.to_string(),
    ))
    .content(file_metadata.clone())
    .await?;
...
pub async fn download_file(
    ...
    sink.send(ClientMessages::RequestFile {
        file_metadata: file_metadata.clone(),
    })
    .await
    ...
    sink.send(ClientMessages::RequestProof {
        file_metadata: file_metadata.clone(),
        columns_to_verify: column_indices_to_verify.clone(),
    })
    .await
    .expect("Failed to send message to server");
    ...
    let CommitOrLeavesOutput::Leaves(leaves_to_verify) =
        convert_file_data_to_commit::<Blake3, WriteableFt63>(
            &encoded_file_data,
        ...
        bail!("Unexpected result from file conversion to Leaves")
    };
    ...
    tracing::debug!(
        "client: writing file to disk at {}",
...
pub fn get_column_indicies_from_random_seed(
    ...
    let cols_to_verify: Vec<usize> = vec_of_all_columns
        .iter()
...
pub async fn verify_compact_commit(
    ...
    tracing::debug!(
        "sending proof request for {} to server",
        &file_metadata.filename
    );
    ...
    let encoded_file_data = convert_byte_vec_to_field_elements_vec(&file_data);
    let CommitOrLeavesOutput::Leaves::<Blake3, WriteableFt63>(locally_derived_leaves) =
        ...
        bail!("Unexpected failure to extract leaves from local file.")
    };
    ...
    D: Digest,
{
    ...
    sink.send(ClientMessages::RequestPolynomialEvaluation {
        file_metadata: file_metadata.clone(),
        evaluation_point,
    })
    .await
    ...
    D: Digest,
{
    ...
    sink.send(ClientMessages::RequestFileReshape {
        file_metadata: file_metadata.clone(),
        ...
        || (new_file_metadata.num_columns != new_pre_encoded_columns)
    {
        ...
        sink.send(ClientMessages::ReshapeResponse {
            new_file_metadata,
...

    {
        ...
        let create_result = db
            .create::<Option<FileMetadata>>((
                constants::CLIENT_METADATA_TABLE,
                new_file_metadata.id_ulid.to_string(),
            ))
...
#[tracing::instrument]
pub async fn delete_file(file_metadata: FileMetadata, server_ip: String) -> Result<()> {
    tracing::info!("requesting deletion of file from server");
    let mut stream = TcpStream::connect(&server_ip).await.unwrap();
    let (mut stream, mut sink) = wrap_stream::<ClientMessages, ServerMessages>(stream);

    sink.send(ClientMessages::DeleteFile {
        file_metadata: file_metadata.clone(),
    })
    .await
    .expect("Failed to send message to server");
...
pub async fn append_to_file(
    ...
    sink.send(ClientMessages::AppendToFile {
        file_metadata: file_metadata.clone(),
        append_data: data_to_append.clone(),
    })
    .await
    .expect("Failed to send message to server");
        ...
        || (file_metadata.num_encoded_columns != appended_file_metadata.num_encoded_columns)
    {
        ...
        sink.send(ClientMessages::EditOrAppendResponse {
            ...
            old_file_metadata: appended_file_metadata,
            accepted: false,
        })
        .await
        .expect("Failed to send message to server");
    ...
    if (appended_file_metadata.filesize_in_bytes
        != file_metadata.filesize_in_bytes + data_to_append.len())
    {
    ...
    let original_polynomial_degree =
        file_metadata.filesize_in_bytes / (WriteableFt63::CAPACITY / 8) as usize;
    ...
    let mut byte_difference_between_evaluations =
        Vec::with_capacity(data_to_append.len() + WriteableFt63::CAPACITY as usize);
    if did_coefficient_change {
        let mut changed_coefficient: WriteableFt63 =
            edited_unencoded_row[original_polynomial_degree % file_metadata.num_columns].clone();

        let original_coefficient_bytes =
            convert_field_elements_vec_to_byte_vec(&[changed_coefficient], byte_offset);
        byte_difference_between_evaluations.extend(&original_coefficient_bytes);
        let original_coefficient =
            convert_byte_vec_to_field_elements_vec(&original_coefficient_bytes);
        ...
        expected_difference_between_evaluations -=
    ...
    tracing::debug!(
        "Expected difference between evaluations: {:?}",
        &expected_difference_between_evaluations
    );
    tracing::debug!(
        "Actual difference between evaluation: {:?}",
    ...
    sink.send(ClientMessages::EditOrAppendResponse {
        ...
        old_file_metadata: appended_file_metadata.clone(),
        accepted: true,
    })
    .await
    .expect("Failed to send message to server");
    ...
    db.create::<Option<FileMetadata>>((
        constants::CLIENT_METADATA_TABLE,
        appended_file_metadata.id_ulid.to_string(),
    ))
    .content(appended_file_metadata.clone())
    .await?;
...
pub async fn edit_file(
    ...
    let data_to_append_len = data_to_append.len();
        ...
        data_to_append_len + edit_start_location <= file_metadata.filesize_in_bytes,
    ...
    tracing::info!(
        "requesting edit to file from server; file: {} @ server: {}",
        file_metadata.filename,
        server_ip
    );
    let mut stream = TcpStream::connect(&server_ip).await.unwrap();
    ...
    .await
    .expect("failed to send initial edit message to server");
    ...
    let ServerMessages::CompactCommit {
        ...
        edited_file_metadata.filesize_in_bytes == file_metadata.filesize_in_bytes,
        "file unexpectedly changed shape on edit"
    );
    ensure!(
        edited_file_metadata.filename == file_metadata.filename,
        "file unexpectedly changed name on edit"
    );
    ensure!(
        edited_file_metadata.num_rows == file_metadata.num_rows,
        "file unexpectedly changed number of rows on edit"
    );
    ensure!(
        edited_file_metadata.num_columns == file_metadata.num_columns,
        "file unexpectedly changed number of columns on edit"
    );
    ensure!(
        edited_file_metadata.num_encoded_columns == file_metadata.num_encoded_columns,
        "file unexpectedly changed number of encoded columns on edit"
    );
    ensure!(
        edited_file_metadata.root.root != file_metadata.root.root,
    ...
    let first_edited_row =
        (edit_start_location / file_metadata.num_columns) / (WriteableFt63::CAPACITY as usize / 8);
    let last_edited_row = ((edit_start_location + data_to_append_len) / file_metadata.num_columns)
        / (WriteableFt63::CAPACITY as usize / 8);
    ...
    let ServerMessages::EditEvaluation {
        ...
        edited_unencoded_rows.len() >= data_to_append_len,
        "server sent insufficient data to verify polynomial"
    );
    ensure!(
        edited_unencoded_rows.len()
            % (WriteableFt63::BYTE_CAPACITY as usize * file_metadata.num_columns)
            == 0,
    ...
    let end_byte_offset_within_coefficient =
        (edit_start_location + data_to_append_len) % (WriteableFt63::BYTE_CAPACITY as usize);
    ...
    expected_edited_bytes.splice(
        ...
        expected_edited_bytes.len() == edited_unencoded_rows.len(),
    ...
    let new_coefficients = fields::convert_byte_vec_to_field_elements_vec(&expected_edited_bytes);
    ensure!(
        original_coefficients.len() == new_coefficients.len(),
    ...
    //  wrong in that case though. Keeping this around for now since it's not what I'm doing
        ...
        new_coefficients != original_coefficients,
        "no data was edited"
    );
    ...
    tracing::debug!(
        ...
        *&new_results - &old_results
        ...
        old_results + expected_difference == new_results,
    ...
    .content(edited_file_metadata.clone())
    .await?;
    db.delete::<Option<FileMetadata>>((
        constants::CLIENT_METADATA_TABLE,
        file_metadata.id_ulid.to_string(),
...
#[tracing::instrument]
pub async fn get_client_metadata_from_database_by_filename(
    filename: &String,
    ...
    db.use_ns(constants::CLIENT_NAMESPACE)
        .use_db(constants::CLIENT_DATABASE_NAME)
        .await?;
    let mut result: Vec<FileMetadata> = db
        .query("SELECT * FROM $table WHERE filename = $filename LIMIT 2")
    ...
    if result.is_empty() {
        ...
        if result.len() > 1 {
            tracing::warn!(
                "Multiple files with the same filename found in database, \
