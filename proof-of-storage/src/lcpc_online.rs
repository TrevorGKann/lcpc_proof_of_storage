    ...
    D: Digest + FixedOutputReset,
{
    ...
    match what_to_extract {
        CommitRequestType::Commit => {
            let commit = LcCommit::<D, LigeroEncoding<F>>::commit(&field_data, &encoding)?;
        ...
        CommitRequestType::Leaves(requested_leaves) => {
            ...
            // now compute FFTs
            encoded_coefs
                .par_chunks_mut(num_encoded_columns)
            ...
            for row in 0..num_matrix_rows {
                for (column_index, hasher) in requested_leaves.iter().zip(digests.iter_mut()) {
        ...
        CommitRequestType::ColumnsWithPath(requested_columns) => {
            let commit = LigeroCommit::<D, F>::commit(&field_data, &encoding)?;

            let columns = requested_columns
                .iter()
                .map(|&column_index| {
                    open_column::<D, LigeroEncoding<F>>(&commit, column_index).unwrap()
                })
...
pub fn server_retreive_columns(comm: &PoSCommit, requested_columns: &Vec<usize>) -> Vec<PoSColumn> {
...
pub fn client_online_verify_column_paths_without_full_columns(
    ...
    for ((col_num, column_path), column_digest) in requested_columns
        .iter()
...
pub fn client_online_verify_column_leaves(
    ...
    if locally_derived_column_leaves.len() != requested_columns.len()
        || received_column_leaves.len() != requested_columns.len()
    {
        return Err(VerifierError::NumColOpens);
    }

    let leaves_ok = locally_derived_column_leaves
        .iter()
    ...
    for (client_leaf, server_leaf) in locally_derived_column_leaves
        .iter()
        .zip(received_column_leaves.iter())
        .take(5)
    {
        tracing::trace!(
            "client leaf: {:x}, server leaf: {:x}",
            client_leaf,
            server_leaf
        );
    }
...
pub fn client_verify_commitment_without_full_columns(
    ...
    client_online_verify_column_leaves(
        locally_derived_column_leaves,
        requested_columns,
        received_column_digests,
    )?;
    client_online_verify_column_paths_without_full_columns(
        commitment_root,
        requested_columns,
        received_column_digests,
        received_column_paths,
    )?;
    ...
    D: Digest,
{
    ...
    for (received_column, received_value) in received_columns
        .iter()
        .zip(result_values_that_match_column_indices.iter())
    {
        let expected_result =
            fields::vector_multiply(left_evaluation_column, &received_column.col[..]);
...
pub fn _encode_row<F: PrimeField>(
    ...
    row_to_encode.extend_from_slice(&row);
    row_to_encode.extend(std::iter::repeat(F::ZERO).take(encoded_size - row_to_encode.len()));
...
fn encode_then_decode_row() {
    ...
    assert_eq!(
        commit.coeffs,
        decode_row(commit.comm).unwrap()[..commit.coeffs.len()]
    )
}
...
fn verify_polynomial_eval() {
    ...
    let (data_realized_width, matrix_colums, _) =
        get_aspect_ratio_default_from_field_len(coefs.len());

    let encoding =
        LigeroEncoding::<WriteableFt63>::new_from_dims(data_realized_width, matrix_colums);
